...
const MainTableLayout: FC = () => {
  ...
  const onChange = (selectedRowKeys: Key[], selectedRows: TRole[]) => {};
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idRole!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idRole!]));
    }
  };

  const onRow = (record: TRole) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idRole!)) {
            newKeys?.push(record.idRole!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter((idKey) => idKey !== record.idRole);
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
