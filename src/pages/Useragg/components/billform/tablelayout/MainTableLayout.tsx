...
const MainTableLayout: FC = () => {
  ...
  const onChange = (selectedRowKeys: Key[], selectedRows: TUser[]) => {};
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idUser!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idUser!]));
    }
  };

  const onRow = (record: TUser) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idUser!)) {
            newKeys?.push(record.idUser!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter((idKey) => idKey !== record.idUser);
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
