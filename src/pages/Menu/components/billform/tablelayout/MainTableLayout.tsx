...
const MainTableLayout: FC = () => {
  ...
  const onChange = (selectedRowKeys: Key[], selectedRows: TMenu[]) => {};
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map((r) => r.idMenu!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idMenu!]));
    }
  };

  const onRow = (record: TMenu) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idMenu!)) {
            newKeys?.push(record.idMenu!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter((idKey) => idKey !== record.idMenu);
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
