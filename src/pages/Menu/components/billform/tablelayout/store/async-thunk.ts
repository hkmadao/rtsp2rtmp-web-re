  ...
  async (message: TMessage, thunkAPI) => {
    ...
    if (selectedTreeNode) {
      const treeIdFn: TFilterNode = equalFilterNode(
        'idParent',
        stringFilterParam(selectedTreeNode['idMenu']),
      );
      fns.push(treeIdFn);
    }
    ...
    const pageInfo: TPageInfo<TMenu> = await ListAPI.pageList(params);
    return {
      selectedTreeNode,
      pageInfo,
    };
...
export const reflesh = createAsyncThunk(
  ...
  async (params: void, thunkAPI) => {
    ...
    if (state.selectedTreeNode) {
      const treeIdFn: TFilterNode = equalFilterNode(
        'idParent',
        stringFilterParam(state.selectedTreeNode['idMenu']),
      );
      fns.push(treeIdFn);
    }
    ...
    const pageInfo: TPageInfo<TMenu> = await ListAPI.pageList(searchParam);
    return pageInfo;
  },
);
...
export const pageChange = createAsyncThunk(
  ...
  async (params: { page: number; pageSize: number }, thunkAPI) => {
    ...
    const pageInfo: TPageInfo<TMenu> = await ListAPI.pageList(queyrParams);
    return pageInfo;
  },
);
