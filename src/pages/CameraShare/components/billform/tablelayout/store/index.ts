...
import { fetchByTreeNode, search, reflesh, pageChange, batchRemove, playAuthCodeReset, statusChange } from './async-thunk';
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(statusChange.fulfilled, (state, action) => {
        ...
        const cameraShare = action.payload;
        state.tableData = state.tableData?.map(data => {
          if (cameraShare.id === data.id) {
            return cameraShare;
          }
          return data;
        }) || [];
