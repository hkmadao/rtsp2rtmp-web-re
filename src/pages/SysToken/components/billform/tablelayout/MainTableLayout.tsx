...
const MainTableLayout: FC = () => {
  ...
  const handleSelect = (
    ...
    if (rowSelectionType == 'checkbox') {
      const newKeys = selectedRows.map(r => r.idToken!);
      dispatch(actions.setSelectedRowKeys(newKeys));
    } else {
      dispatch(actions.setSelectedRowKeys([record.idToken!]));
    }
  }

  const onRow = (record: TToken) => {
    return {
      onClick: (event: any) => {
        if (rowSelectionType == 'checkbox') {
          let newKeys = tableStore.selectedRowKeys?.slice() || [];
          if (!newKeys.includes(record.idToken!)) {
            newKeys?.push(record.idToken!);
            dispatch(actions.setSelectedRowKeys(newKeys));
          } else {
            newKeys = newKeys?.filter(
              (idKey) => idKey !== record.idToken,
            );
            dispatch(actions.setSelectedRowKeys(newKeys));
          }
