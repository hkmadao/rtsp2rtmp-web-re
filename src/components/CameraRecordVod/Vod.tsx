...
import styles from '../CameraRecordVod/style.less';
...
const Vod: FC<{
  ...
  useEffect(() => {
    const videoDurationUrl = `/cameraRecord/getDuration?idCameraRecord=${idCameraRecord}`;
    setLoading(true);
  ...
  const flv_load = (vodPrp: VodProp) => {
    ...
    let videoUrl = `${Env.directServerUrl}/cameraRecord/start?idCameraRecord=${idCameraRecord}&playerId=${vodPrp.playerId}&seekSecond=${seekSecond}`;
